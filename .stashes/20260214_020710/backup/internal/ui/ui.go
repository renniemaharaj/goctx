...
func Run() {
	...
	win.SetTitle("GoCtx Manager v2.5")
...
func renderDiff(p model.ProjectOutput) {
	...
	for _, path := range keys {
		...
		getTag := func(n string) *gtk.TextTag { t, _ := statsBuf.GetTagTable().Lookup(n); return t }
	...
	if len(keys) > limit {
		...
		tag, _ := statsBuf.GetTagTable().Lookup("warn")
		statsBuf.InsertWithTag(statsBuf.GetEndIter(), msg, tag)
	}
}
