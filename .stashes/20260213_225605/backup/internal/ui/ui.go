...
func renderDiff(p model.ProjectOutput, title string) {
	...
	for path, newContent := range p.Files {
		...
		for _, d := range diffs {
			...
			switch d.Type {
			case diffmatchpatch.DiffInsert: statsBuf.InsertWithTag(end, d.Text, getTag("added"))
			case diffmatchpatch.DiffDelete: statsBuf.InsertWithTag(end, d.Text, getTag("deleted"))
			default: statsBuf.Insert(end, d.Text)
			}
		}
	}
}

func getTag(n string) *gtk.TextTag { t, _ := statsBuf.GetTagTable().Lookup(n); return t }
